...
2.  Compute $\kappa_0 = \langle k^2 \rangle / \langle k \rangle$ and determine whether the network has a giant component.
3.  Compute the critical threshold $f_c$ under random node removal using $f_c = 1 - 1/(\kappa_0 - 1)$.
4.  Now consider a second network with degree distribution concentrated on $\{1, 2, 7\}$ with $p_1 = 0.4$, $p_2 = 0.3$, $p_7 = 0.3$ (note: this gives a higher average degree $\langle k \rangle = 3.1$). Compute $\kappa_0$ and $f_c$ for this network. Even after accounting for the higher mean degree, explain why the much larger $\langle k^2 \rangle$ — driven by degree heterogeneity — is the main reason $f_c$ increases so substantially.

---

#### Exercise 2. Erdős--Rényi threshold derivation and bond percolation

1.  Starting from the Poisson degree distribution, derive $\kappa_0 = \langle k \rangle + 1$ for ER networks.
2.  Derive the critical threshold $f_c^{\text{ER}} = 1 - 1/\langle k \rangle$ under random node removal.
3.  Now consider bond percolation: edges are removed independently with probability $q$. Show that for an ER network, the bond percolation threshold is $q_c = 1 - 1/\langle k \rangle$. Explain why site and bond thresholds coincide for ER networks.
4.  Compute $f_c$ numerically for $\langle k \rangle \in \{2, 3, 5, 10\}$ and interpret the trend.

---

#### Exercise 3. Robustness index

Let $S(f)$ denote the giant component size under removal fraction $f$, and define $R = \int_0^1 S(f)\,df$.

1.  Suppose a network has a piecewise-linear robustness curve $S(f) = (1 - f/f_c)_+$ where $(x)_+ = \max(x, 0)$. Show that $R = f_c/2$.
2.  A second network has $S(f) = (1 - f)^2$ for $0 \leq f \leq 1$. Compute its robustness index and compare with the piecewise-linear model.
3.  Explain why $R$ alone is not sufficient to characterize a network's robustness. What additional information does the full robustness curve $S(f)$ provide?

---

#### Exercise 4. Scale-free robustness and the Achilles heel

1.  Provide a mathematical explanation of why scale-free networks with $2 < \gamma \leq 3$ have $f_c \to 1$ under random node removal. In your answer, discuss the role of $\langle k^2 \rangle$ diverging for such exponents and the low probability of randomly hitting a hub.
2.  Explain why these same networks are extremely vulnerable to targeted attack. What happens to $\kappa_0$ when the highest-degree hubs are removed first?
3.  A network engineer proposes: *"We should build our communication network with a scale-free topology because it is the most robust."* Write a short critical response identifying the flaw in this reasoning and discussing the design tension between error tolerance and attack vulnerability.

---

#### Exercise 5. Cascading failures and beyond the giant component

1.  Explain qualitatively how a cascading failure differs from the simple percolation model. In your answer, address: (a) what triggers a cascade; (b) why networks near their percolation threshold are most vulnerable; (c) one real-world example.
2.  The giant component size $S(f)$ is the standard robustness observable, but it does not capture everything. Discuss at least two additional observables (e.g. average path length, network efficiency, second-largest component size) and explain what information each adds beyond $S(f)$.
3.  Compare the qualitative robustness profiles of ER, WS, and BA networks under both random failure and targeted attack. Organize your answer in terms of degree distribution shape, expected $f_c$, and transition sharpness.
...
1.  Generate an Erdős--Rényi network ($N = 400$, $\langle k \rangle = 4$) and a Barabási--Albert network ($N = 400$, $m = 2$). For each, simulate random node removal at fractions $f = 0, 0.02, 0.04, \ldots, 0.98$, recording $S(f)$. Average over at least 5 realisations and plot the two robustness curves on the same figure. Overlay the theoretical $f_c^{\text{ER}}$ as a vertical dashed line.
2.  Repeat the experiment using targeted attack (removing nodes in decreasing order of degree, recalculating degrees after each removal). Plot the targeted-attack curves for both networks on a second figure.
3.  Compute the robustness index $R$ for all four combinations (ER random, ER targeted, BA random, BA targeted) and present the results in a table. Discuss the asymmetry.

---

#### Exercise 7. Betweenness attack and finite-size effects

1.  Modify the targeted attack from Exercise 6 to remove nodes in decreasing order of betweenness centrality (recalculating after each removal). For the BA network ($N = 300$, $m = 2$), plot robustness curves under degree-based attack and betweenness-based attack on the same figure. Which strategy is more destructive?
2.  Repeat the random-failure robustness simulation for ER networks with $\langle k \rangle = 4$ and sizes $N \in \{200, 500, 1000\}$. Plot all three curves on one figure and explain why the percolation transition becomes sharper as $N$ increases.
3.  For $N = 500$, plot the size of the second-largest component as a function of $f$ alongside the giant component curve. Show that the second-largest component peaks near the critical threshold and explain the significance of this observation.
