...
plot_degree_colored <- function(g, main_text) {
  ...
  cols <- cut(
    ...
    breaks = unique(brks),
    include.lowest = TRUE,
    labels = c(course_cols$blue, course_cols$teal, course_cols$gold)[seq_len(length(unique(brks)) - 1)]
  )
...
ggplot(cum_tbl, aes(x = k, y = ccdf)) +
  geom_point(color = course_cols$gold, size = 2.0, alpha = 0.8) +
  scale_x_log10() +
  scale_y_log10() +
  labs(
...
ggplot(compare_tbl, aes(x = k, y = prob, color = model)) +
  ...
  scale_x_log10() +
  scale_y_log10() +
  scale_color_manual(values = c("Barabasi-Albert" = course_cols$gold, "Erdos-Renyi" = course_cols$teal)) +
  labs(
