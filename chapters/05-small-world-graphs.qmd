...
The seven exercises below cover the full content of this chapter. They require derivation, interpretation, and computation — not reproduction of results already in the text. The first three are mathematical, the next two analytical, and the final two computational.

---

#### Exercise 1. From branching to logarithmic distance

The branching approximation estimates the number of vertices reachable within distance $d$ by

$$
N(d)\approx 1+\langle k \rangle+\langle k \rangle^2+\cdots+\langle k \rangle^d.
$$

1.  Derive the closed-form expression
    $$
    N(d)\approx \frac{\langle k \rangle^{d+1}-1}{\langle k \rangle-1}
    $$
    for $\langle k \rangle \neq 1$. State clearly the modeling assumption under which this approximation is meaningful.
2.  Starting from $N(d_{\max})\approx N$ and the large-$N$ dominance of the final term, derive
    $$
    d_{\max}\approx \frac{\ln N}{\ln \langle k \rangle}.
    $$
    Explain each algebraic step carefully.
3.  The relation $L(G)\approx \ln N / \ln \langle k \rangle$ is only an approximation. Explain why it is not exact. Your answer should discuss neighborhood overlap, loops and finite-size effects, and the difference between average distance and diameter.
4.  A popular slogan says that "everyone is only a few steps apart." Explain why this statement should not be read as a fixed constant independent of system size. Distinguish carefully between a constant bound and logarithmic growth.

---

#### Exercise 2. Lattice scaling and the surprise of small worlds

1.  Show that in a $d$-dimensional regular lattice with $N$ vertices, the characteristic linear size is of order $N^{1/d}$. Explain why this leads to
    $$
    L(G)\sim D(G)\sim N^{1/d}.
    $$
2.  Write a short mathematical comparison of the growth laws
    $$
    \ln N, \qquad N^{1/3}, \qquad N^{1/2}, \qquad N.
    $$
    For $N = 10^6$, compute numerical values of each expression (use natural logarithms for $\ln N$) and comment on the gaps.
3.  Explain why the small-world effect is surprising from the viewpoint of low-dimensional lattice geometry. What changes conceptually when distances grow logarithmically rather than polynomially?

---

#### Exercise 3. The regular ring lattice: degree, distance, and clustering

Let $G$ be the regular ring lattice with parameters $(N,K)$, where $K$ is even and each vertex is joined to its $K/2$ nearest neighbors on each side.

1.  Prove that every vertex has degree $K$. Deduce that $|E| = NK/2$ and that the average degree is $\langle k \rangle = K$.
2.  Give a heuristic derivation of the estimate
    $$
    L(0)\approx \frac{N}{2K}.
    $$
3.  Derive the clustering formula
    $$
    C(0)=\frac{3(K-2)}{4(K-1)}.
    $$
    *(Hint: fix a vertex $0$, write $m = K/2$, and count the edges among its $K$ neighbors on the ring. Then divide by $\binom{K}{2}$.)*
4.  Evaluate $C(0)$ for $K \in \{4,6,10\}$. What value does $C(0)$ approach as $K \to \infty$? Interpret this limit.

---

#### Exercise 4. Shortcuts, clustering, and normalized observables

Let $C(\beta)$ and $L(\beta)$ be the clustering coefficient and average path length in the Watts--Strogatz model with parameters $(N,K,\beta)$.

1.  Define the normalized quantities $C(\beta)/C(0)$ and $L(\beta)/L(0)$. Explain why these ratios are useful when studying the transition from order to randomness.
2.  Explain why a small number of long-range shortcuts can drastically reduce average path length while leaving clustering comparatively intact. Use the language of local versus long-range edges and graph distance.
3.  State the behavior expected for the two normalized quantities in the small-world regime, and relate your statement to @def-small-world-regime.
4.  A student claims: *"Since both an Erdős--Rényi graph and a Watts--Strogatz graph can have short paths, they are essentially the same model."* Identify what is wrong with this claim. Address clustering, local order, and global reachability.

---

#### Exercise 5. Interpreting small-worldness and the limits of Watts--Strogatz

1.  Recall the small-worldness index
    $$
    \sigma = \frac{C/C_{\mathrm{rand}}}{L/L_{\mathrm{rand}}}.
    $$
    Explain what $\sigma > 1$ means structurally. Why is it not enough to examine clustering alone?
2.  Discuss why the Watts--Strogatz model does not naturally explain hub-dominated degree distributions or strong degree-dependent clustering. Which modeling ingredient is missing?
3.  A network analyst reports $\sigma = 4.2$ for a social network with $N = 500$ and $\langle k \rangle = 8$. Another analyst replies that this "proves" the network was generated by Watts--Strogatz rewiring. Critically assess that reply.

---

#### Exercise 6. Distance scaling in random networks

Fix an average degree $\langle k \rangle = 6$. For several values of $N$ in $\{50, 100, 200, 400, 800\}$:

1.  Generate connected realisations of $G(N,p)$ with $p = \langle k \rangle/(N-1)$ (reject disconnected samples). For each $N$, average the mean path length over at least $10$ independent connected realisations.
2.  Plot the empirical average path length against $N$, and overlay the theoretical curve
    $$
    L(G)\approx \frac{\ln N}{\ln \langle k \rangle}.
    $$
3.  Comment on the agreement. Where is the approximation better, and why might it deviate at small $N$?

---

#### Exercise 7. Identifying and quantifying the small-world regime

Use $N = 400$ and $K = 8$ throughout.

1.  Simulate the Watts--Strogatz model on a grid of rewiring probabilities spanning several orders of magnitude (include $\beta = 0$). For each $\beta$, average $C(\beta)$ and $L(\beta)$ over several connected realisations. Plot
    $$
    \frac{L(\beta)}{L(0)}
    \qquad\text{and}\qquad
    \frac{C(\beta)}{C(0)}
    $$
    on the same axes (log scale in $\beta$ is recommended). Identify the small-world regime visually.
2.  Construct three graphs of comparable size and average degree: a ring lattice ($\beta = 0$), a Watts--Strogatz graph with small $\beta$ in the small-world regime, and a connected Erdős--Rényi graph with $p = K/(N-1)$. Compute clustering, average path length, and $\sigma$ for each. Organise the results in a table.
3.  Interpret the table: which model combines high clustering with short paths? What does $\sigma$ add beyond looking at $C$ or $L$ separately?
