...
theme_set(
  theme_minimal(base_size = 13) +
    theme(
      ...
      axis.title = element_text(color = course_cols$ink, face = "plain"),
      axis.text = element_text(color = course_cols$ink),
      axis.ticks = element_line(color = course_cols$grid, linewidth = 0.3),
      plot.title = element_text(face = "bold", color = course_cols$ink, size = rel(1.12)),
      plot.subtitle = element_text(color = course_cols$slate, margin = margin(b = 10)),
      plot.caption = element_text(color = course_cols$slate, hjust = 1, size = rel(0.86)),
      legend.position = "top",
      legend.title = element_text(color = course_cols$ink, face = "bold"),
      legend.text = element_text(color = course_cols$slate),
      legend.key.width = unit(1.2, "lines"),
      strip.text = element_text(face = "bold", color = course_cols$ink),
      strip.background = element_rect(fill = scales::alpha(course_cols$blue, 0.08), color = NA),
      plot.background = element_rect(fill = "white", color = NA),
      panel.background = element_rect(fill = "white", color = NA),
      plot.margin = margin(10, 12, 8, 6)
    )
)
...
plot(
  ...
  edge.color = scales::alpha(course_cols$blue, 0.8),
  edge.width = 2.2,
  main = "Example graph"
)
```
...
ggplot(degree_count_table, aes(x = degree, y = proportion)) +
  geom_col(fill = course_cols$gold, alpha = 0.55, width = 0.8) +
  geom_point(color = course_cols$blue, size = 2.8) +
...
#| echo: false
#| fig-cap: "Three example neighborhoods for the same focal node A. dark green edges connect A to its neighbors; light green edges connect neighbors to each other and contribute to clustering."
...
plot_clustering_example <- function(edges_center, edges_neighbor, title_text) {
  ...
  plot(
    ...
    vertex.color = c(course_cols$gold, course_cols$blue, course_cols$blue, course_cols$blue),
    vertex.frame.color = "white",
    vertex.label.color = "white",
    vertex.label.cex = 1,
